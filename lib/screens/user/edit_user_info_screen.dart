...
class _EditUserInfoScreenState extends State<EditUserInfoScreen> {
                              ...
                              onImageSelected: (file) {
                                ...
                                if (_serviceImages.length >= _maxServiceImages) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(content: Text('Chỉ được thêm tối đa 3 ảnh dịch vụ')),);
                                  return;
                                }
                          ...
                          children: [
                             SmartImagePicker(label: 'Thêm chứng chỉ', showPreview: false, onImageSelected: (file) {
                               ...
                               if (_certificateImages.length >= _maxCertificateImages) {
                                 ScaffoldMessenger.of(context).showSnackBar(
                                   const SnackBar(content: Text('Chỉ được thêm tối đa 1 ảnh chứng chỉ')),);
                                 return;
                               }
                          ...
                          ClipOval(
                            child:
                                _avatarFile != null
                                    ...
                                    : (widget.userInfo.avatar != null && widget.userInfo.avatar!.isNotEmpty)
                                    ? (resolveImageUrl(widget.userInfo.avatar!) != null
                                        ? Image.network(resolveImageUrl(widget.userInfo.avatar!)!, fit: BoxFit.cover)
              ...
              ListTile(
                ...
                onTap: () async {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text(ok ? 'Tải ảnh đại diện thành công' : 'Tải ảnh đại diện thất bại'),
                  ));
  ...
  Future<void> _updateGarageInfo() async {
    // 1. Cập nhật thông tin gara
    final garageData = {
      'name_garage': _nameCtrl.text, // Sử dụng _nameCtrl cho tên gara
    ...
    // 2. Cập nhật chứng chỉ (nếu có thay đổi)
    ...
    // 3. Cập nhật file đăng ký (nếu có thay đổi)
