...
class _EditUserInfoScreenState extends State<EditUserInfoScreen> {
  ...
  void initState() {
    ...
    // Lấy thông tin mới nhất từ UserProvider thay vì widget.userInfo
    final userProvider = Provider.of<UserProvider>(context, listen: false);
    final currentUserInfo = userProvider.userInfo ?? widget.userInfo;

    // Kiểm tra nếu là gara thì sử dụng nameGarage, không thì dùng name
    if (userProvider.isGarageUser) {
      _nameCtrl.text = currentUserInfo.nameGarage ?? currentUserInfo.name;
    } else {
      _nameCtrl.text = currentUserInfo.name;
    }

    _phoneCtrl.text = currentUserInfo.phone;
    _addressCtrl.text = currentUserInfo.address ?? '';
    _descCtrl.text = currentUserInfo.descriptionGarage ?? '';
    _fromCtrl.text = _extractYear(currentUserInfo.createdAt);

    // Initialize garage-specific fields
    _emailCtrl.text = currentUserInfo.emailGarage ?? '';
    _workerCtrl.text = currentUserInfo.numberOfWorker ?? '';
    _servicesCtrl.text = currentUserInfo.servicesProvided ?? '';

    // Initialize current files from user info
    _currentServiceFiles = currentUserInfo.listFileAvatar ?? [];
    _currentCertificateFiles = currentUserInfo.listFileCertificate ?? [];
    ...
    for (int i = 0; i < _currentServiceFiles.length; i++) {
      // debugPrint('[EditUserInfoScreen] Service file $i: id=${_currentServiceFiles[i].id}, name=${_currentServiceFiles[i].name}, path=${_currentServiceFiles[i].path}');
    }

    for (int i = 0; i < _currentCertificateFiles.length; i++) {
      // debugPrint('[EditUserInfoScreen] Certificate file $i: id=${_currentCertificateFiles[i].id}, name=${_currentCertificateFiles[i].name}, path=${_currentCertificateFiles[i].path}');
    }
  }
          ...
          Positioned(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  GestureDetector(
                    onTap: _isUploadingAvatar ? null : _showAvatarPickerSheet,
                    child: SizedBox(
                      width: 64,
                      height: 64,
                      child: Stack(
                          ...
                          ClipOval(
                            child: Consumer<UserProvider>(
                              builder: (context, userProvider, child) {
                                ...
                                // Sau đó ưu tiên ảnh từ UserProvider (đã được cập nhật)
                                final currentUserInfo = userProvider.userInfo;
                                if (currentUserInfo?.avatarPath != null && currentUserInfo!.avatarPath!.isNotEmpty) {
                                  final imageUrl = resolveImageUrl(currentUserInfo.avatarPath!);
                                  if (imageUrl != null) {
                                    return Image.network(imageUrl, fit: BoxFit.cover);
                                  }
                                }

                                // Fallback về widget.userInfo (chỉ khi UserProvider chưa có data)
                                if (userProvider.userInfo == null &&
                                    widget.userInfo.avatarPath != null &&
                                    widget.userInfo.avatarPath!.isNotEmpty) {
                                  final imageUrl = resolveImageUrl(widget.userInfo.avatarPath!);
                                  if (imageUrl != null) {
                                    return Image.network(imageUrl, fit: BoxFit.cover);
                                  }
                                }

                                return Container(color: DesignTokens.surfaceBrand);
                              },
                            ),
                          ),
                          // Lớp phủ đen + icon camera hoặc loading
                          Container(
                            decoration: BoxDecoration(
                              color:
                                  _isUploadingAvatar ? Colors.black.withOpacity(0.6) : Colors.black.withOpacity(0.35),
                              shape: BoxShape.circle,
                            ),
                            ...
                            child: _isUploadingAvatar
                                ...
                                : const SvgIcon(svgPath: 'assets/icons_final/camera.svg', size: 18),
                          ),
                        ],
                      ),
                    ),
                  ...
                  const SizedBox(height: 12),
                  MyTextField(controller: _nameCtrl, obscureText: false, hasError: false),
                ],
              ),
            ),
          ),
            ...
            children: [
              ListTile(
                ...
                onTap: () async {
                  ...
                  try {
                    debugPrint('[EditUserInfoScreen] Starting avatar upload...');
                    debugPrint('[EditUserInfoScreen] File path: ${result!.path}');
                    debugPrint('[EditUserInfoScreen] File exists: ${await result!.exists()}');

                    final ok = await UserService.uploadAvatar(result!);
                    debugPrint('[EditUserInfoScreen] Upload result: $ok');

                    if (!mounted) return;

                    if (ok) {
                      debugPrint('[EditUserInfoScreen] Upload successful, refreshing user info...');
                      // Cập nhật UserProvider với thông tin mới sau khi upload thành công
                      final userProvider = Provider.of<UserProvider>(context, listen: false);
                      await userProvider.refreshUserInfo();
                      _hasChanges = true;

                      // Trigger rebuild để hiển thị avatar mới
                      setState(() {});

                      AppToastHelper.showInfo(context, message: 'Tải ảnh đại diện thành công');
                    } else {
                      debugPrint('[EditUserInfoScreen] Upload failed');
                      AppToastHelper.showInfo(context, message: 'Tải ảnh đại diện thất bại');
                    }
                  } catch (e, stackTrace) {
                    debugPrint('[EditUserInfoScreen] Upload error: $e');
                    debugPrint('[EditUserInfoScreen] Stack trace: $stackTrace');
                    if (mounted) {
                      AppToastHelper.showError(context, message: 'Lỗi upload: $e');
                    }
                  } finally {
                    // Kết thúc loading
                    if (mounted) {
              ...
              if (_avatarFile != null && !_isUploadingAvatar)
                ListTile(
  ...
  Future<void> _onSubmit() async {
    try {
      ...
      if (mounted) {
        AppToastHelper.showSuccess(context, message: 'Cập nhật thông tin thành công!');
      }
  ...
  bool _hasUserChanges() {
    // Lấy thông tin mới nhất từ UserProvider
    final userProvider = Provider.of<UserProvider>(context, listen: false);
    final currentUserInfo = userProvider.userInfo ?? widget.userInfo;

    // So sánh tên hiện tại với tên ban đầu
    final originalName = currentUserInfo.name;
  ...
  bool _hasGarageChanges() {
    ...
    final originalNameGarage = currentUserInfo.nameGarage ?? currentUserInfo.name;
    final currentNameGarage = _nameCtrl.text.trim();

    final originalEmailGarage = currentUserInfo.emailGarage ?? '';
    final currentEmailGarage = _emailCtrl.text.trim();

    final originalAddress = currentUserInfo.address ?? '';
    final currentAddress = _addressCtrl.text.trim();

    final originalWorker = currentUserInfo.numberOfWorker ?? '';
    final currentWorker = _workerCtrl.text.trim();

    final originalDesc = currentUserInfo.descriptionGarage ?? '';
    final currentDesc = _descCtrl.text.trim();

    final originalServices = currentUserInfo.servicesProvided ?? '';
  ...
  Future<void> _updateUserInfo() async {
    ...
    if (_hasUserChanges()) {
      final updateData = {'name': _nameCtrl.text};
  ...
  List<Map<String, dynamic>> _getCurrentCertificateFiles() {
    return _currentCertificateFiles.map((file) => {'id': file.id, 'path': file.path}).toList();
  }

  // Lấy danh sách file đăng ký hiện tại
  List<Map<String, dynamic>> _getCurrentServiceFiles() {
    return _currentServiceFiles.map((file) => {'id': file.id, 'path': file.path}).toList();
  }
  ...
  Widget _buildCertificateImagesDisplay() {
    ...
    for (int i = 0; i < _certificateImages.length; i++) {
      ...
      allImages.add(
        _fileThumb(_certificateImages[i], () {
